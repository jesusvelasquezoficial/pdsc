...
function pctj24H(precios) {
    let precio = precios[precios.length - 1];
    let precioAnterior = precios[precios.length - 2];
    let pctjVariacion = ((precio - precioAnterior) / precioAnterior) * 100;
    pctjVariacion = pctjVariacion.toFixed(2);
    let var_24H = sube0baja(pctjVariacion);
    var_24H.unshift(pctjVariacion);
    return var_24H;
}
    ...
    async loadDataPetroleo(context) {
        await Axios.get("http://pdsc.phoenixplus.net:4000/api/oil").then(async res => {
            ...
            await data.forEach((datos, index) => {
                // Damos formato a la fecha y hacemos flotante el precio (dolar)
                let fecha = moment(datos.fecha).format("L");
                let precio = parseFloat(datos.dolar.replace(".", "").replace(",", "."));
                let pcjtDolar = porcentajeDolar(data, datos, index);
                pcjtDolar = parseFloat(pcjtDolar[0]);
                // almacenamos en las listas de fechas y precios
                fechas[index] = fecha;
                precios[index] = precio;
                preciosPctj[index] = pcjtDolar;
            ...
            // Almacenamos un Arreglo de Precios
            context.commit('setDataPetroleo', precios);

            // Calculamos el % de Variacion 24H
            let var_24H = pctj24H(precios);
            ...
            let precio_min = Math.min(...precios);
            let precio_max = Math.max(...precios);
            ...
            // Precio Actual
            let precio_actual = precios[precios.length - 1];
            ...
            context.commit('petroleoFecha', fechas);
            // Base predeterminada 
            context.commit('minPetroleoX', fechas[fechas.length - 7]); // modificar esto
            context.commit('minPetroleoY', base_min_y);

            graphGeneral.fechas = fechas;
            graphGeneral.datos = preciosPctj;
            graphGeneral.indicadores = indicador;
            graphGeneral.escala.x.min = fecha_min;
            graphGeneral.escala.x.max = fecha_max;
            graphGeneral.escala.y.min = precio_min;
            graphGeneral.escala.y.max = precio_max;
            context.commit('setGraficoGeneral', graphGeneral);
...
const mutations = {
    setGraficoGeneral(state, graficoGeneral) {
        graficoGeneral.loaded = true;
        state.graficoGeneral = graficoGeneral;
    },
