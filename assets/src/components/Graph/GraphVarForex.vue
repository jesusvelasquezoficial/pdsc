  ...
  <div>
    <app-card v-if="loadedTasas" :heading="cardTitulo" :headingMenu="cardMenu">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <!-- Indicador -->
          <div class="col-4 col-sm-3 col-md-2 p-2" v-for="(indicador, index) in indicadoresVariacionForex" :key="index">
            <!-- Color -->
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('datos', ['indicadoresVariacionForex']),
    dataGraficoGeneral(){
      let datos = [];
      this.indicadoresVariacionForex.forEach((indicador, index) => {
        datos.push({
  ...
  methods: {
    cargarDatos(){
        console.log(this.indicadoresVariacionForex);
        
        this.indicadoresVariacionForex.forEach(indicador => {
