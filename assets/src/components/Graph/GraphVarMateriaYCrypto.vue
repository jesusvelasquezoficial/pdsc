  ...
  <div>
    <app-card v-if="loadedTasas" :heading="cardTitulo" :headingMenu="cardMenu">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <!-- Indicador -->
          <div class="col-4 col-sm-3 col-md-2 p-2" v-for="(indicador, index) in indicadoresVariacionMateriaYCrypto" :key="index">
            <!-- Color -->
...
export default {
  ...
  data() {
    return {
      cardMenu: true,
      cardTitulo:'Variacion de Materias Primas y Cryptomonedas',
      fechasBase: [],
      datosBase:[],
  ...
  computed: {
    ...
    ...mapGetters('datos', ['indicadoresVariacionMateriaYCrypto']),
    dataGraficoGeneral(){
      let datos = [];
      this.indicadoresVariacionMateriaYCrypto.forEach((indicador, index) => {
        datos.push({
  ...
  methods: {
    cargarDatos(){
        console.log(this.indicadoresVariacionMateriaYCrypto);
        
        this.indicadoresVariacionMateriaYCrypto.forEach(indicador => {
